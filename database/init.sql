...
CREATE TABLE ml_prediction_results (
    ...
    user_id UUID REFERENCES users(id),
    document_id UUID REFERENCES documents(id),
    predicted_decision TEXT CHECK (predicted_decision IN ('approve', 'decline')) NOT NULL,
...
CREATE TABLE decision_log (
    ...
    final_decision TEXT CHECK (final_decision IN ('approve', 'decline', 'pending', 'escalated')) NOT NULL,
    explanation TEXT,
    fairness_flag BOOLEAN DEFAULT FALSE,
    created_at TIMESTAMP DEFAULT NOW()
);
